...
function Category() {
  ...
  const toggleText = (id) => {
    setExpandedItems((prev) => ({
      ...prev,
      [id]: !prev[id], // Toggle the expanded state for the given item
    }));
  };
  ...
  useEffect(() => {
    ...
    setSelectedCategoryName(localStorage.getItem("category_name") || "");
  ...
  const handleChooseClick = () => {
    ...
    if (selectedProduct.varients.length > 0) {
      ...
      setIsVariationPopupVisible(true); // Show the variation popup
    } else {
      setIsCharacteristicsPopupVisible(true); // Show the characteristics popup
      setVarients([]); // Reset the varients
      setSelectedVariation(""); // Reset the selected variation
    }
  ...
  const handleSelectVariation = (id) => {
    ...
    setIsVariationPopupVisible(false); // Close the variation popup
    setIsCharacteristicsPopupVisible(true); // Show the characteristics popup
  ...
  return (
    <>
      ...
      <section className="container mx-auto pb-32">
        ...
        <div className="px-4 mt-6 border-b border-neutral-100 pb-2">
        ...
        <div className=" px-4 mt-6">
          {items
            ...
            .map((item) => (
              <div className="flex gap-2 border-b border-gray-200 pb-6 mb-4">
                <div className="w-7/12">
                  ...
                  <h1 className="text-base font-semibold mt-1">{item.name}</h1>
                  <div className="flex gap-2 items-center">
                    <p className="text-base font-semibold">R {item.price}</p>
                  </div>
                        ...
                        {item.properties.map((taste) => (
                          <button
                            ...
                            className={`px-4 py-2 rounded text-sm ${
                              selectedCharacters[item.character] === taste
